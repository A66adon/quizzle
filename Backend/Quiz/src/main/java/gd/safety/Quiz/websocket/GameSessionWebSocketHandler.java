...
import gd.safety.Quiz.session.GameSessionRegistry.AnswerRejectedException;
import gd.safety.Quiz.session.GameSessionRegistry.AnswerSubmissionResult;
import gd.safety.Quiz.session.GameSessionRegistry.JoinNotAllowedException;
...
import gd.safety.Quiz.websocket.PlayerNamePolicy.InvalidPlayerNameException;
import gd.safety.Quiz.websocket.WebSocketConnectionHub.ClientConnection;
import gd.safety.Quiz.websocket.WebSocketConnectionHub.UnbindResult;
import gd.safety.Quiz.websocket.WebSocketProtocol.JoinRequest;
import gd.safety.Quiz.websocket.WebSocketProtocol.AnswerRequest;
import gd.safety.Quiz.websocket.WebSocketProtocol.ProtocolException;
	...
	protected void handleTextMessage(WebSocketSession socket, TextMessage message) {
		...
		if (connection.authenticated()) {
			boolean receivedWhileQuestionOpen = sessionRegistry.find(connection.codehash())
					.map(snapshot -> snapshot.state() == gd.safety.Quiz.session.GameState.QUESTION_OPEN)
					.orElse(false);
			handleAnswer(
