...
import org.dev.quizzle.session.GameSessionRegistry.AnswerRejectedException;
import org.dev.quizzle.session.GameSessionRegistry.AnswerSubmissionResult;
import org.dev.quizzle.session.GameSessionRegistry.JoinNotAllowedException;
...
import org.dev.quizzle.websocket.PlayerNamePolicy.InvalidPlayerNameException;
import org.dev.quizzle.websocket.WebSocketConnectionHub.ClientConnection;
import org.dev.quizzle.websocket.WebSocketConnectionHub.UnbindResult;
import org.dev.quizzle.websocket.WebSocketProtocol.JoinRequest;
import org.dev.quizzle.websocket.WebSocketProtocol.AnswerRequest;
import org.dev.quizzle.websocket.WebSocketProtocol.ProtocolException;
	...
	protected void handleTextMessage(WebSocketSession socket, TextMessage message) {
		...
		if (connection.authenticated()) {
			boolean receivedWhileQuestionOpen = sessionRegistry.find(connection.codehash())
					.map(snapshot -> snapshot.state() == GameState.QUESTION_OPEN)
					.orElse(false);
			handleAnswer(
